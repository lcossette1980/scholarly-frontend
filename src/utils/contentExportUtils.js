...
/**
 * Creates document footer with page numbers, branding, and date
 */
  ...
  return new Footer({
    children: [
      new Paragraph({
        children: [
          new TextRun({
            text: `Generated by DraftEngine | ${currentDate}`,
...
/**
 * Creates a Document Summary page with meta description and social excerpt
 */
const createDocumentSummaryPage = (metaDescription, socialExcerpt) => {
  const paragraphs = [];
  ...
  // Social Media Excerpt heading
  if (socialExcerpt) {
    paragraphs.push(
      new Paragraph({
        children: [
          new TextRun({
            text: 'Social Media Excerpt',
            font: STYLES.fonts.heading,
            size: 28,
            color: STYLES.colors.primary,
            bold: true
...
/**
 * Creates a References section from a references list
 */
const createReferencesSection = (referencesList) => {
  const paragraphs = [];
  ...
  // Handle references as array of strings or a single string block
  const refs = Array.isArray(referencesList) ? referencesList : referencesList.split('\n').filter(r => r.trim());

  refs.forEach((ref) => {
    const refText = typeof ref === 'string' ? ref.trim() : (ref.formatted || ref.text || String(ref));
    if (!refText) return;
 ...
 */
const parseContentToParagraphs = (content, sectionImages = {}) => {
  const paragraphs = [];
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('## ')) {
      ...
      // Check for a section image placeholder
      const sectionKey = String(sectionIndex);
      const imageEntry = sectionImages[sectionKey] || sectionImages[sectionIndex];
      if (imageEntry) {
        const prompt = typeof imageEntry === 'string' ? imageEntry : (imageEntry.imagePrompt || imageEntry.prompt || '');
        if (prompt) {
          paragraphs.push(
            new Paragraph({
              children: [
...
export const exportGeneratedContent = async (job, options = {}) => {
  ...
  try {
    // Use refined title if available, fall back to outline title or default
    const title = job.refinedTitle || job.refined_title || job.outline?.title || 'Generated Content';
    const content = job.content || '';

    // Extract enhanced output fields
    const metaDescription = job.metaDescription || job.meta_description || null;
    const socialExcerpt = job.socialExcerpt || job.social_excerpt || null;
    const referencesList = job.referencesList || job.references_list || job.references || null;
    const sectionImages = job.sectionImages || job.section_images || {};
    ...
    // Add Document Summary page with meta description and social excerpt
    if (metaDescription || socialExcerpt) {
      children.push(...createDocumentSummaryPage(metaDescription, socialExcerpt));
    }

    // Parse and add content with section image placeholders
    children.push(...parseContentToParagraphs(content, sectionImages));

    // Add references section at the end
    if (referencesList && (Array.isArray(referencesList) ? referencesList.length > 0 : referencesList.trim())) {
      children.push(
        new Paragraph({
          children: [new PageBreak()]
        })
      );
      children.push(...createReferencesSection(referencesList));
    }
